...
class Logger(object):
    def __init__(self,
        ...
        self._perf_mg = MetersGroup(os.path.join(log_dir, 'perf'),
                                    formating=COMMON_PERF_FORMAT)
    ...
    def log(self, key, value, step, n=1, log_frequency=1):
        ...
        assert key.startswith('train') or key.startswith('eval') or key.startswith('perf')
    ...
    def dump(self, step, save=True, ty=None):
        if ty is None:
            ...
            self._perf_mg.dump(step, 'perf', save)
        elif ty == 'eval':
            self._eval_mg.dump(step, 'eval', save)
        elif ty == 'train':
            self._train_mg.dump(step, 'train', save)
